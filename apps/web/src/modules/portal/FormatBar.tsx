 ...
 * to learn Markdown to answer a client. The syntax is still what gets
...
const ACTIONS: Array<{ label: string; title: string; format: Format }> = [
  { label: 'B', title: 'Bold', format: 'strong' },
  { label: 'I', title: 'Italic', format: 'em' },
  { label: '</>', title: 'Code', format: 'code' },
  { label: '\u{1F517}', title: 'Link', format: 'link' },
  { label: '\u2022', title: 'List', format: 'bullet' },
];
...
export function FormatBar({
  ...
  return (
    <div className="formatbar" role="group" aria-label="Formatting">
      {ACTIONS.map((action) => (
        <button
          key={action.label}
          type="button"
