      ...
      inputSchema: z.object({
        ...
        markdown: z.string().describe(
          [
            ...
            '',
            'Colour, in exactly two forms and no others:',
            '<span style="color:#d33">red text</span> and',
            '<mark style="background-color:#ffd400">a coloured highlight</mark>.',
            'The colour must be a hex literal. Any other tag, attribute or colour spelling is',
            'refused and would otherwise appear as literal angle brackets mid-sentence.',
            '',
            'Prefer **bold** and ==highlight== for ordinary emphasis; reach for colour when the',
            'colour itself carries the meaning, such as a risk or a status.',
            '',
            'There is still no underline: Markdown cannot express it in any form.',
          ].join('\n'),
        ),
        section: z
          .string()
