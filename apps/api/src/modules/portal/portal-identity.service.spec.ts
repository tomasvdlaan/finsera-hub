...
/**
 * The login-time checks, without a live Zitadel.
 *
 * This ran on every request as the portal guard until Phase 8 moved it to the login
 * callback; the tests moved with it unchanged, which is the evidence that the checks did.
...
describe('PortalIdentityService', () => {
  ...
  let bySubject: ReturnType<typeof vi.fn>;
  let memberWithEmail: ReturnType<typeof vi.fn>;
  let guard: PortalIdentityService;

  beforeEach(() => {
    resolveFromSubject = vi.fn();
    // Nobody is a member unless a test says so — the ordinary case is a client signing in.
    bySubject = vi.fn().mockResolvedValue(undefined);
    memberWithEmail = vi.fn().mockResolvedValue(false);
  ...
  it('refuses every request when the portal audience is unset', async () => {
    ...
    await expect(guard.identify('a.b.c')).rejects.toThrow(/not configured/i);
  ...
  it('rejects an opaque token instead of reporting a generic failure', async () => {
    ...
    await expect(guard.identify('opaque-token')).rejects.toThrow(/Opaque access token/);
  });
  ...
  it('never resolves a visitor from a token it could not verify', async () => {
    ...
    await expect(guard.identify('not.a.jwt')).rejects.toThrow(/Invalid token/);
    expect(resolveFromSubject).not.toHaveBeenCalled();
  });
  ...
  describe('once the signature and audience are accepted', () => {
    ...
    it('falls back to the invitation when the role check is off', async () => {
      ...
      expect(await guard.identify('a.b.c')).toMatchObject({ clientId: 'c-1' });
    });
    ...
    it('still refuses an employee with the gate off, via the invitation', async () => {
      ...
      await expect(guard.identify('a.b.c')).rejects.toThrow(/No portal access/);
    ...
    it('recognises an employee by their core.users row, before asking about clients', async () => {
      ...
      bySubject.mockResolvedValue({ id: 'u-1', email: 'tomas@finsera.nl' });

      expect(await guard.identify('a.b.c')).toEqual({
        kind: 'staff', staffUserId: 'u-1', email: 'tomas@finsera.nl',
      });
      // Never asked. An employee who also held an invitation must not resolve as that
      // client — their view of a portal is a colleague's, audited under their own name.
      expect(resolveFromSubject).not.toHaveBeenCalled();
    });

    it('refuses an employee whose internal role was taken away', async () => {
      delete process.env.PORTAL_ROLE_CHECK;
      accepts({ sub: 'sub-employee' });
    ...
    it('recognises an employee with the role gate off, on the row alone', async () => {
      ...
      // G6 is why this matters today: Zitadel emits no roles, so the row is the evidence.
      expect(await guard.identify('a.b.c')).toMatchObject({ kind: 'staff', staffUserId: 'u-1' });
    });

    it('never lets a colleague’s address claim a client invitation', async () => {
      process.env.PORTAL_ROLE_CHECK = 'off';
      accepts({ sub: 'sub-new-colleague' });
      // Not yet in core.users by subject — they have never signed in to the internal app.
      resolveFromSubject.mockRejectedValue(new Error('No portal access'));
      memberWithEmail.mockResolvedValue(true);
      global.fetch = vi.fn().mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ email: 'new@finsera.nl', email_verified: true }),
      }) as never;

      // The gap bySubject cannot close: hired, given a Zitadel account, and their address
      // happens to match a pending invitation. Claiming it would bind them to that client.
      await expect(guard.identify('a.b.c')).rejects.toThrow(/No portal access/);
      expect(memberWithEmail).toHaveBeenCalledWith('new@finsera.nl');
    });
