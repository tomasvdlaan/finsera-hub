...
describe('opening an invitation', () => {
  ...
  const build = (authRequestId: string | null) => {
    ...
    const oidc = {
      beginLogin,
      authRequestIdFor: vi.fn().mockResolvedValue(authRequestId),
    } as unknown as PortalOidcService;

    const zitadel = {
      zitadelInviteUrl: (userId: string, code: string, id: string | null) =>
        `${ISSUER}/ui/login/user/invite?userID=${userId}&code=${code}` +
        (id ? `&authRequestID=${id}` : ''),
  ...
  it('sends the client to Zitadel carrying a request for it to finish', async () => {
    const { controller, req, res, redirect } = build('auth-req-1');
    ...
    expect(url.searchParams.get('authRequestID')).toBe('auth-req-1');
  ...
  it('remembers the login, so the callback can finish it', async () => {
    const { controller, req, res, beginLogin } = build('auth-req-1');
  ...
  it('still opens the invitation when Zitadel will not mint a request', async () => {
    ...
    expect(location).toContain('/ui/login/user/invite');
    expect(location).not.toContain('authRequestID');
    // No cookie: there is no login in flight to remember.
    expect(res.cookie).not.toHaveBeenCalled();
  });

  it('refuses a link with nothing on it', async () => {
    const { controller, req, res } = build('auth-req-1');
